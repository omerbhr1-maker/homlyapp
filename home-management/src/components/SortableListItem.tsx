...
export const SortableListItem = memo(function SortableListItem({
  ...
  const commitEdit = useCallback(() => {
    ...
    if (normalized && normalized !== item.text) {
      onEdit(sectionKey, item.id, normalized);
    }
  }, [editText, item.text, item.id, sectionKey, onEdit]);
  ...
  return (
    <li
      ref={setNodeRef}
      style={{ transform: CSS.Transform.toString(transform), transition, willChange: isDragging ? "transform" : "auto" }}
      ...
      {isEditing ? (
        <>
          <input
            ...
            onBlur={commitEdit}
            dir="rtl"
            className="min-w-0 flex-1 bg-transparent text-sm text-slate-700 dark:text-slate-200 outline-none border-b-2 border-teal-500 dark:border-teal-400 py-1"
          />
          <button type="button" onMouseDown={(e) => { e.preventDefault(); commitEdit(); }} aria-label="שמור" className="flex h-8 w-8 items-center justify-center rounded-xl text-teal-600 dark:text-teal-400 transition hover:bg-teal-50 dark:hover:bg-teal-900/30">
            <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
              <path d="M20 6 9 17l-5-5" />
            </svg>
          </button>
          <button type="button" onMouseDown={(e) => { e.preventDefault(); cancelEdit(); }} aria-label="בטל" className="flex h-8 w-8 items-center justify-center rounded-xl text-slate-400 dark:text-slate-500 transition hover:bg-slate-100 dark:hover:bg-slate-700">
            <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <>
          ...
          {showActions ? (
            <>
              <button type="button" onClick={startEdit} aria-label="עריכה" className="flex h-8 w-8 items-center justify-center rounded-xl text-slate-600 dark:text-slate-300 transition hover:bg-slate-100 dark:hover:bg-slate-700">
                <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
